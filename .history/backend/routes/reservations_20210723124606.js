...
router.put("/reservation/:id", (request, response, next) => {
  const updatedReservation = new reservationTemplateCopy({
    name: request.body.name,
    email: request.body.email,
    phone: request.body.phone,
    pax: request.body.pax,
  ...
  ReservationModel.findByIdAndUpdate(request.params.id, updatedReservation, {
    new: true,
  })
    .then((updatedRev) => {
